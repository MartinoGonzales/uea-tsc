...
public class MaxClassifier extends StatsClassifier implements Serializable {
    ...
    private double [][][] maxMatrix;
    
    
    public MaxClassifier(int numberClasses, int noFeat, int noDays, boolean overallStatsF, boolean standardiseF) {
        if (overallStatsF)
            this.clsID = "MaxClass_Overall";
        else 
            this.clsID = "MaxClass";
        this.noFeat = noFeat;
        ...
        this.maxMatrix = new double[numberClasses][][];
        for (int c= 0; c < maxMatrix.length; c++) {
            if (isOverall) 
                this.maxMatrix[c] = new double[noFeat][1];
            else
                this.maxMatrix[c] = new double[noFeat][noDays];
        }
    }
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        for (int cls = 0; cls < temp.numClasses(); cls++) {
            ...
            for (int atr = 0; atr < subSet.numAttributes()-1; atr++) {
                ...
                if (this.isOverall) {
                    if (dayIndex == 0)
                        this.maxMatrix[cls][featIndex][0] = stats.max;
                    else 
                        if (stats.min < this.maxMatrix[cls][featIndex][0]) 
                            this.maxMatrix[cls][featIndex][0] = stats.max;
                }
                else
                    this.maxMatrix[cls][featIndex][dayIndex] = stats.max;
            }
    ...
    public double [] getClassDistances (Instance inst) {
        ...
        for (int atr = 0; atr < tempInst.numAttributes()-1; atr++) {
            ...
            for (int cls = 0; cls < inst.numClasses(); cls++) {
                double dist =0;
                if (this.isOverall)
                    dist = (tempInst.value(atr) - this.maxMatrix[cls][featIndex][0]) * (tempInst.value(atr) - this.maxMatrix[cls][featIndex][0]);
                else 
                    dist = (tempInst.value(atr) - this.maxMatrix[cls][featIndex][dayIndex]) * (tempInst.value(atr) - this.maxMatrix[cls][featIndex][dayIndex]);
                dists[cls] += dist;
            }
        }
        return dists;
